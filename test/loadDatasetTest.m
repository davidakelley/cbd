classdef loadDatasetTest < matlab.unittest.TestCase
    %LOADDATASETTEST is the test suite for cbd.loadDataset
    %
    % David Kelley, 2015
    ...
    methods (Test)
        ...
        function testLoad(testCase)
            dataset = cbd.loadDataset( ...
                {'LR', 'GDPH', 'C02'}, 'Display', 'none');
        ...
        function testMany(testCase)
            specs = {'GDPH', ...
                ...
                'SA( FTO) / GOVPRICE@LOCAL', ...
                'BFGR(BGSB/10000,TMBCA)', ...
                '((NARI+NAWIH)/EXTEND_LAST(EXTEND((NWIH+NRI)/(NWIH+NMI+NRI),"endCount",1)))/PB9411', ...
                'FFGR(SPLICE(TITH, TITH2),ADVINV@LOCAL)', ...
                'IGTP', ...
                'YPMH', ...
                '(FFP3-FTBS3)#dbID:WEEKLY', ...
                'CBILL@LOCAL + FTBS3@WEEKLY', ...
                'ffp@local-FTBS3@WEEKLY'};
            
            names = {'GDP', 'GDI', 'PCE', 'HOURS', 'CU', 'HOUST', ...
                'GOVPRICE', 'RFTO',  'TRADE', 'ADVINV', 'RMTI', 'IP', ...
                'PI', 'CBILL', 'FFP', 'FTBS'};
        ...
        function testNames(testCase)
            % If there's no computation to a series, we want to use the 
            % name of the series (minus any database name).
