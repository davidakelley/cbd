classdef (Abstract) sourceseries < matlab.unittest.TestCase
    ...
    %
    % DESCRIPTION
    %   The sourceseries class is the parent class for all tests of the
    %   cbd.private.(source).m functions, which are in test/(source).m.
    %   This class contains the properties (options) and methods (tests)
    %   that all cbd.private.(source).m functions must pass
    %
    % USAGE
    %   >> runtests('haverseries') % execute haverseries tests
    %
    % SEE ALSO: HAVERSERIES, BLOOMBERGSERIES, FREDSERIES, CHIDATASERIES
    %
    ...
    methods (TestClassSetup)
        function baseOpts(tc)
            % Set the expected options for all testfun
            tc.opts.startDate = [];
            tc.opts.endDate = [];
        end % function
    end % methods
