function [output, seriesProp] = datapull(strIn, opts)
%DATAPULL Evaluate a cbd data expression
%
...
if ~isempty(operatorDiv)
    ...
    for iOp = 1:length(operators)
        args = breakOnChar(strIn, operators(iOp));
        if length(args) > 2
        ...
        if length(args) == 2
            ...
            for iArg = 1:2
                [arguments{iArg}, seriesProps{iArg}] = ...
                    cbd.private.datapull(args{iArg}, opts);
            end
...
elseif ~isempty(argumentDiv)
    ...
    [output, seriesProp] = cbd.private.datapull(mainStr, opts);
...
elseif ~isempty(fnRegex) 
    ...
    assert(~isempty(openParens), 'data:datapull:parens', ...
...
elseif ~isempty(strfind(strIn, '"'))
    ...
    assert(length(split) <= 2, 'data:datapull:invalidInput', ...
...
function args = breakOnChar(argStr, breakChar)
...
assert(mod(length(quoteChars), 2) == 0, 'cbd:data:mismatchedString', ...
