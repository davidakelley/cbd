...
switch upper(freq)
    ...
    case 'M'
        ...
        e1Year = 13 - cbd.month(sDate);                          % index of the end of first year
        sLYear = (length(monthList)-cbd.month(eDate) + 1);       % index of the start of last year
        ...
        yearList(wholeYears) = sort(repmat((cbd.year(sDate)+1:cbd.year(eDate)-1)', 12, 1));
    ...
    case 'Q'
        ...
        e1Year = 5-cbd.quarter(sDate);                          % index of the end of first year
        sLYear = (length(quartList)-cbd.quarter(eDate) + 1);    % index of the start of last year
        ...
        yearList(wholeYears) = sort(repmat((cbd.year(sDate)+1:cbd.year(eDate)-1)', 4, 1));
        
        dates = cbd.private.endOfMonth(yearList, 3 * quartList);
    case 'A'
        yearList = cbd.year(sDate):cbd.year(eDate);
        dates = cbd.private.endOfMonth(yearList, repmat(12, [size(yearList, 1), 1]));
    otherwise
